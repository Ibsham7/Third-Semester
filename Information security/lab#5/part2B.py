...
def string_to_bit_blocks(text):
    ...
    return [list(map(int, bits[i:i+64])) for i in range(0, len(bits), 64)]
...
def desEncryption(plaintext, key64):
    ...
    for block in blocks:
        ...
        combined = R + L
        encrypted_blocks.append(permute(combined, FP))
...
def desDecryption(encrypted_blocks, key64):
    ...
    for block in encrypted_blocks:
        ...
        combined = R + L
        decrypted_blocks.append(permute(combined, FP))
...
if __name__ == "__main__":
    ...
    print("Ciphertext (first block bits):", ''.join(str(b) for b in encrypted_blocks[0]))

    print("\nDecrypting...")
    decrypted_text = desDecryption(encrypted_blocks, key64)
    print("Decrypted plaintext:", repr(decrypted_text))
